#!/usr/bin/env python3
"""Build integrated predictor-importance heatmaps across 27 configurations.

The input may be a PySR equation-usage summary or a seed-aggregated SHAP
summary. The same top-k union, column order, clustering, and publication style
are therefore reusable across every model in the project.
"""
    ...
    for task in TASK_ORDER:
        for family in FAMILY_ORDER:
            for scenario in SCENARIO_ORDER:
                ...
                # 9-column block, so keep each column label on a single line.
                config_label = f"{predictor} | {FAMILY_TITLE[family]}"
                configs.append(
                    ConfigInfo(
                        family=family,
                        task_type=task,
...
def load_importance_results(
    ...
    """Load PySR usage or SHAP summaries into a standardized tidy table."""
    ...
    is_pysr = pysr_required.issubset(base.columns)
    is_shap = shap_required.issubset(base.columns)
    if not is_pysr and not is_shap:
        missing_pysr = sorted(pysr_required - set(base.columns))
        missing_shap = sorted(shap_required - set(base.columns))
        raise ValueError(
            "Input is neither a PySR usage summary nor a SHAP summary. "
            f"Missing PySR columns: {missing_pysr}; missing SHAP columns: {missing_shap}"
        )
    if is_shap:
        base = base[base["model"].astype(str).eq(model_name)].copy()
    ...
    for _, row in base.iterrows():
        ...
        if is_pysr:
            ...
            n_replicates = int(row["n_seed_equations"])
            n_present = int(row["n_seeds_using_feature"])
            frequency_present = float(row["seed_presence_rate"]) if "seed_presence_rate" in row else np.nan
            source = str(row.get("source", ""))
        else:
            importance_mean = float(row["mean_abs_shap"])
            importance_median = float(row.get("median_abs_shap", importance_mean))
            importance_std = float(row["sd_abs_shap"]) if not pd.isna(row["sd_abs_shap"]) else np.nan
            n_replicates = int(row["n_seeds"])
            n_present = n_replicates
            frequency_present = min(1.0, n_replicates / 100.0)
            source = str(row.get("source_type", row.get("source", "")))
        rows.append(
            {
                "model": model_name,
                "task_type": task,
                ...
                "n_replicates_present": n_replicates,
                "n_seeds_using_feature": n_present,
...
def plot_importance_heatmap(
    ...
    cmap: str = "viridis",
    importance_label: str = "Predictor importance",
) -> pd.DataFrame:
    ...
    cbar.set_label(f"{importance_label} ({normalize})", fontsize=8)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--importance-label", default=None)
...
def main() -> None:
    args = parse_args()
    importance_label = args.importance_label
    if importance_label is None:
        importance_label = "PySR predictor importance" if args.model_name == "pysr" else "Mean absolute SHAP value"
