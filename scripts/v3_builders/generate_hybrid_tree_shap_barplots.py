...
def main() -> None:
    ...
    summary.to_csv(tables / f"{args.model}_hybrid_shap_predictor_importance_summary.csv", index=False)
    source_cov = (
    ...
    source_cov.to_csv(tables / f"{args.model}_hybrid_shap_source_coverage.csv", index=False)
    ...
    manifest.to_csv(tables / f"{args.model}_hybrid_plot_manifest.csv", index=False)
    metadata = {
        ...
        "note": "Uses seed-level combined SHAP when present; otherwise falls back to direct shap_importance/mean_abs_shap_by_feature.csv.",
    }
